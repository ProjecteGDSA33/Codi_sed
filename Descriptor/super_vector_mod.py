...
class Imatge:
  ...
  sol = ''      # soluciÃƒÂ³ del fitxer de solucions
...
for foto in root:
    ...
    if imatge.iden+".jpg" in nom_imatges:    
        ...
        # un cop ja guardem la soluciÃƒÂ³ l'eliminem de la llista per reduir temps de cerca
        sol_imatges = sol_imatges[sol_imatges.document_id != imatge.iden]
        imatge.sol = sol_trobada.event_type.to_string().split()[-1].encode('ascii','ignore')   # guardem la soluciÃƒÂ³ en la imatge
