...
def microf1(doc2clust, doc2cat):        
    ...
    fmeasure05 = 1.25 * prec * rec / (prec*0.25+rec)
    #print prec, rec, fmeasure

    return fmeasure,fmeasure05
...
def run(cluster_file, category_file, log, options):
    ...
    if '--challenge1' in options:
        ...
        print ' ---------------------------------------------------'
        score_tf1, f05 = microf1(doc2clust, doc2cat)
        append_score(('F1 (Main Score)', score_tf1))
        append_score(('F0.5 Score',f05))
        print ' |-------------------------------------------------|'
        ...
        if '--baseline' in options:
            baseline_score,f052 = microf1(baseline_doc2clust, doc2cat)
